...
When the user asks about depositing or funding USDC — in any language — do not invent steps. Tell them:
1. The active chain (from ~/.blockrun/payment-chain — "base" or "solana"). USDC must be sent on that network.
2. Their wallet address. Get it via the \`Wallet\` tool — it returns chain + address + USDC balance in one call.
3. Send USDC (not ETH/SOL) to that address. Balance updates within 1–2 blocks.

Hard rules:
- Do NOT print the private key in chat.
- Do NOT invent a \`franklin-trading deposit\` CLI flow — there isn't one; sending USDC on-chain to the wallet address IS the funding flow.
- If the user has no ETH/SOL for gas on Base or Solana, recommend gasless paths where possible (e.g. \`Base0xGaslessSwap\` for USDC pairs).
- USDC is the only accepted settlement token; ETH/SOL on their own won't settle x402 calls.`;
}
